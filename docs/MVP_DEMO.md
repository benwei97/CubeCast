...
On Home, competition cards should be compact links above the market feed. Click one to see that contest's complete competition market list, with no filters. Click a market and use Back to return to the competition, then Back again to Home. Opening either detail URL directly in a new tab should make Back return to Home. `/competitions` no longer has a directory page. Adding legacy category/status query parameters must not filter the list or bypass contest visibility.
...
2. On a market preview, check prominent probabilities/score swings and the competitor stats beneath them. Personal-best averages and average world ranks come from WCA records. Recent results and Market rules should be visible without clicking. Stats may stream in later or show a retry notice; odds must stay unchanged.
...
6. Existing markets should show real WCA PBs without regeneration. Internal generation metadata is retained but no longer displayed as a duplicate comparison. No technical cache/model explanations should appear on the page; raw settlement evidence is admin-only.
