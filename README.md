...
Phase 4 settlement and leaderboard is complete: admins can resolve, void, or tie V1 markets, settlement snapshots and audit records are stored, valid 10-pick entries are scored from 1,000 points, and `/leaderboard` ranks contest entries.

Phase 5 admin contest tools are complete: admins can review generated contests, configure diversity caps, and publish generated draft V1 markets.

Phase 6 WCA integration is complete: WCA OAuth can be enabled with credentials, WCA identities are stored on login, and admins can refresh result snapshots for WCA-linked competitions.
...
Phase 8 legacy database cleanup is complete: purchase, position, ledger, balance, liquidity, and old settlement schema have been removed in migration `20260915230000_phase8_legacy_db_cleanup`.
...
Phase 14 WCA recommendation pipeline is complete: admins can generate a draft weekly contest from the largest upcoming WCA competitions, preview competitor limits and top ranked entrants, and review 10 generated draft markets per competition before publishing.
...
6. Sign in with WCA, promote your user to admin in the database, then use `/admin` to generate a contest from real upcoming WCA competitions.
